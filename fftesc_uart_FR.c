...
typedef enum
{
  UART_OBTAIN_DATA_ONCE = 0,              // Obtenir les données actuelles
  UART_CONTROL_AND_OBTAIN_DATA_ONCE = 2,
  UART_SET_DUTY = 3,                      // Définir le rapport cyclique
  UART_SET_CURRENT = 4,                   // Définir le courant
  UART_SET_CURRENT_GEAR = 5,              // Définir le courant avec position de vitesse
  UART_SET_BRAKE_CURRENT = 6,             // Définir le courant de freinage
  UART_CAN_FWARD = 16,
  UART_OBTAIN_FIRMWARE_VERSION = 17,      // Version du logiciel
  UART_KEEP_LIVE = 25,
  UART_SET_AUTO_OBTAIN_REALTIME_DATA = 26,
  UART_RESET_AND_REBOOT_FTESC = 29,
  UART_OBTAIN_ALL_FTESC_ID = 30,
  UART_SET_CURRENT_GEAR_AND_OBTAIN_DATA = 32, // Définir le courant
  UART_REBOOT_FTESC = 35,
  UART_SET_ID_CURRENT = 37,               // Définir le courant
...
// Exécuter cette fonction pour envoyer une commande au MCU
void Ftesc_Obtain_Realtime_Data(void)
{
  uint8_t payload[1];
  payload[0] = UART_OBTAIN_DATA_ONCE;
  FTESC_Uart_packSendPayload(payload, 1); // Fonction d'envoi par port série
}

ftesc_realtime_data esc_data;

// En général, on utilise l'interruption de ligne inactive pour signaler la fin de réception série ;
// le tampon uart_rx_buff est transféré via DMA.
// Une fois l'interruption de ligne inactive déclenchée, on peut analyser les données du tampon.
int8_t Ftesc_Uart_processReadPacket(uint8_t *uart_rx_buff)
{
  uint16_t data_len = 0;
  uint8_t *data = 0;
  // Vérifier si l'en-tête de trame est correct
  ...
  {
    data_len |= (uint16_t)uart_rx_buff[1] << 8; // 8 bits de poids fort
    data_len |= (uint16_t)uart_rx_buff[2];      // 8 bits de poids faible
    data = uart_rx_buff + 3;
  }
  else
  {
    return -1; // Format de paquet anormal — doit commencer par 0xAA ou 0xBB
  }

  // Calculer le code de vérification CRC16 des données de la couche liaison
  // et récupérer le code CRC16 reçu
  ...
  // Vérifier si la vérification CRC16 est correcte
  if (data_crc16_calc != data_crc16_rece)
  {
    return -2; // Échec de la vérification CRC16 du paquet
  }

  // Vérifier si la queue de trame est correcte
  if (data[data_len + 2] != 0XDD)
  { // Doit se terminer par 0xDD
    return -3;
  }

  UART_COMMAND uart_cmd = (UART_COMMAND)data[0]; // Récupérer la commande
  uint16_t index = 0;
  data++; // Se décaler vers la zone DATA
   switch (uart_cmd)
   {
   ...
   break;
   case UART_OBTAIN_FIRMWARE_VERSION:
   {
     // Firmware version response: 5 bytes
     // [controller_id, version_major, version_minor, version_patch, model_type]
     data[index++] = 0;  // controller_id (assuming single ESC, always 0)
     data[index++] = FIRMWARE_VERSION_MAJOR;
     data[index++] = FIRMWARE_VERSION_MINOR;
     data[index++] = FIRMWARE_VERSION_PATCH;
     data[index++] = FIRMWARE_MODEL_TYPE;
   }
   break;
   default:
     break;
   }
   return 0; // Analyse de trame terminée
}
